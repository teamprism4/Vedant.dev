...
function HeroAvatar({ innerRef }: { innerRef: React.RefObject<HTMLDivElement | null> }) {
  const [failed, setFailed] = useState(false);
  if (failed) return null;
  return (
    // 👇 ref is now on the element that visually  the avatar
...
export function HeroSection() {
  ...
  const InFollowers = 500;
  const lcTotal = leetcode.data?.total ?? 65;
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center w-full max-w-6xl mx-auto px-4 sm:px-6 pb-24">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center w-full">
          ...
          >
            ...
            >
              Vedant <span className="gradient-text">Garje</span>
            </motion.h1>
            ...
            >
              {[
                ...
                { value: InFollowers, suffix: "+", label: "LinkedIn Followers" },
              ].map((stat) => (
